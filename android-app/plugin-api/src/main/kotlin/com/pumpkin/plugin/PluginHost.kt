...
data class PluginSetting(
    ...
    val kind: Kind,
    /**
     * 用户没设置过时用的值 —— **它同时是界面上的显示值和宿主的实际行为**。
     *
     * 所以这个值**必须与宿主的内置默认一致**。例如「停止前确认」这个键，
     * 宿主本来就默认要确认，插件就得写 `"true"`；要是写成 `"false"`，
     * 界面会显示「关」而 App 其实还在确认 —— 用户点一下以为打开了，实际是关掉。
     *
     * [Kind.TOGGLE] 只有 `"true"` 算开，其余（含默认的 `""`）都算关，
     * 所以**开关类设置项一定要显式写这个字段**。
     */
